...
export async function autolinkAndroid({ dependencies, projectDir, outputModulesJsonPath, outputPackagesJavaPath }: { dependencies: string[]; projectDir: string; outputModulesJsonPath: string; outputPackagesJavaPath: string }) {
  const autolinkingInfo = (await Promise.all(dependencies.map((npmPackageName) => mapPackageNameToAutolinkingInfo({ npmPackageName, projectDir }))))
    .filter((p) => !!p)
    .flat(1)
    .map(({ npmPackageName, sourceDir, androidProjectName, packageImportPath, packageInstance }) => ({
  ...
  await Promise.all([
    await writeModulesJsonFile({
      moduleInfo: autolinkingInfo.map(({ packageName, absolutePath, androidProjectName }) => ({
        packageName,
        absolutePath,
        androidProjectName,
      })),
 ...
 */
async function mapPackageNameToAutolinkingInfo({ npmPackageName, projectDir, userConfig = {} }: { npmPackageName: string; projectDir: string; userConfig?: AndroidDependencyParams }) {
...
async function writeModulesJsonFile({
  ...
  return await writeFile(outputModulesJsonPath, JSON.stringify(moduleInfo, null, 2), { encoding: 'utf-8' });
}
 ...
 *   location.
 */
async function writePackagesJavaFile({ moduleInfo, outputPackagesJavaPath }: { moduleInfo: { packageImportPath: string; packageInstance: string }[]; outputPackagesJavaPath: string }) {
  const contents = ['package com.bridge;', '', 'import com.facebook.react.ReactPackage;', '// Add all imports here:', ...moduleInfo.map(({ packageImportPath }) => packageImportPath), '', 'import java.util.ArrayList;', 'import java.util.Collections;', 'import java.util.List;', '', 'public class Packages {', '  public static List<ReactPackage> list = new ArrayList<>();', '  public static void init() {', '    // Register each package.', '    Collections.addAll(list,', ...moduleInfo.map(({ packageInstance }) => `      ${packageInstance},`), '    );', '  }', '}', ''].join('\n');
